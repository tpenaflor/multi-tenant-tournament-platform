...
export default async function TournamentPage({ params }: TournamentPageProps) {
  ...
  if (!tournament) {
    return (
      <main className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center">
        <h1 className="text-3xl font-bold text-slate-400">Tournament Not Found</h1>
      </main>
    );
  }
  ...
  if (components.length > 0) {
    return (
      <main className="min-h-screen bg-slate-950 text-slate-100 p-8">
        ...
        {isOwner && (
          <a href={`/builder?tournament=${tournament.id}`} className="fixed bottom-8 right-8 z-50 flex items-center gap-2 px-5 py-3 rounded-full bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold shadow-xl shadow-sky-500/20 transition-transform hover:scale-105">
